...
object IsContravariant {
  ...
  final def apply[F[_]](implicit F: IsContravariant[F]): IsContravariant[F] = F

  trait SubstCv[F[_]] extends IsContravariant[F] with Alt[SubstCv[F]] {
  ...
  trait SubstCt[F[_]] extends IsContravariant[F] with Alt[SubstCt[F]] {
  ...
  trait LiftLiskov[F[_]] extends IsContravariant[F] with Alt[LiftLiskov[F]] {
    final override def substCv[G[+ _], A, B](g: G[F[B]])(implicit ev: A <~< B): G[F[A]] =
  ...
  implicit def scalaContravariant[F[- _]]: IsContravariant[F] =
    instanceOf(new IsContravariant.SubstCv[F] {
      def substCv[G[+ _], A, B](g: G[F[B]])(implicit ev: A <~< B): G[F[A]] = {
        type H[-T] = G[F[T]]
        ev.substCt[H](g)
      }
