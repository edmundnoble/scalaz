...
object IsCovariant {
  ...
  final def apply[F[_]](implicit F: IsCovariant[F]): IsCovariant[F] = F

  /** Unsafe witness that the type constructor `F[_]` is covariant. */
  def unsafeForce[F[_]]: IsCovariant[F] =
    instanceOf(new IsCovariant.LiftLiskov[F] {
  ...
  trait SubstCv[F[_]] extends IsCovariant[F] with Alt[SubstCv[F]] {
  ...
  trait SubstCt[F[_]] extends IsCovariant[F] with Alt[SubstCt[F]] {
  ...
  trait LiftLiskov[F[_]] extends IsCovariant[F] with Alt[LiftLiskov[F]] {
    final override def substCv[G[+ _], A, B](g: G[F[A]])(implicit ev: A <~< B): G[F[B]] =
  ...
  implicit def scalaCovariant[F[+ _]]: IsCovariant[F] =
    instanceOf(new IsCovariant.SubstCv[F] {
      def substCv[G[+ _], A, B](g: G[F[A]])(implicit ev: A <~< B): G[F[B]] = {
        type H[+T] = G[F[T]]
        ev.substCv[H](g)
      }
